...
def DenseNetFCN(input_shape, nb_dense_block=3, growth_rate=16, nb_layers_per_block=4,
    ...
    img_input = Input(shape=input_shape)#实例化一个keras张量
...
def __create_fcn_dense_net(nb_classes, img_input, nb_dense_block=5, growth_rate=12,
    ...
    # Initial convolution,init_conv_filters:卷积核的数目(及输出的维度)
    ...
    x2 = Conv2D(init_conv_filters, (5, 5), kernel_initializer='he_normal', padding='same', name='initial_conv2D2',
               kernel_regularizer=l2(weight_decay))(img_input)###
    x = concatenate([x1, x2], axis=concat_axis)  ###拼接
    ...
    skip_down_list.append(x)###x是一个张量 ?，512,512,48   float32
    ...
    for block_idx in range(nb_dense_block):
        ...
        x = __transition_block(x, nb_filter, dropout_rate=dropout_rate,
                               compression=compression, weight_decay=weight_decay)#对应TD块,
    ...
    for block_idx in range(nb_dense_block):
        ...
        t = __transition_up_block(l, nb_filters=n_filters_keep, type=upsampling_type, weight_decay=weight_decay)#?,?,?,160,创新
        t1= __transition_up_block1(l, nb_filters=n_filters_keep, type=upsampling_type, weight_decay=weight_decay)#?,?,?,160 5*5转置卷积
       
        #print(skip_list[block_idx])#?,128,128,304
...
def __conv_block(input, nb_filter, bottleneck=False, dropout_rate=None, weight_decay=1e-4):
    '''
        if bottleneck:
            conv_block = BN + Relu + Conv2D(1*1) + BN + Relu + Conb2D(3*3) + Dropout
        else:
            conv_block = BN + Relu + Conb2D(3*3) + Dropout
    '''
    concat_axis = -1
...
def __dense_block(x, nb_layers, nb_filter, growth_rate, bottleneck=False, dropout_rate=None, weight_decay=1e-4,
    ...
    x_list = [x]
    for i in range(nb_layers): #nb_layers代表当前DB块的层数
...
def __transition_block(input, nb_filter, dropout_rate, compression=1.0, weight_decay=1e-4):
    ...
    x = AveragePooling2D((2, 2), strides=(2, 2))(x)#MaxPooling2D,文章中是最大池化
